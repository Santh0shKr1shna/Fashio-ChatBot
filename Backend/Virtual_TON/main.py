...
if __name__ == '__main__':
    ...
    # Read input image
    img=cv2.imread(f"{VTON_IMG_UPLOAD_DIR}/static/origin_web.jpg")
    ori_img=cv2.resize(img,(768,1024))
    cv2.imwrite(f"{VTON_IMG_UPLOAD_DIR}/origin.jpg",ori_img)

    # Resize input image
    img=cv2.imread(f'{VTON_IMG_UPLOAD_DIR}/origin.jpg')
    img=cv2.resize(img,(384,512))
    cv2.imwrite(f'{VTON_IMG_UPLOAD_DIR}/resized_img.jpg',img)

    # Get mask of cloth
    print("Get mask of cloth\n")
    terminnal_command = f"python {VTON_IMG_UPLOAD_DIR}/get_cloth_mask.py" 
    os.system(terminnal_command)

    # Get openpose coordinate using posenet
    print("Get openpose coordinate using posenet\n")
    terminnal_command = f"python {VTON_IMG_UPLOAD_DIR}/posenet.py" 
    os.system(terminnal_command)
    ...
    os.chdir(f"{VTON_IMG_UPLOAD_DIR}/Graphonomy-master")
    terminnal_command =f"python exp/inference/inference.py --loadmodel ./inference.pth --img_path ../resized_img.jpg --output_path ../ --output_name /resized_segmentation_img"
    os.system(terminnal_command)
    os.chdir("../")

    # Remove background image using semantic segmentation mask
    mask_img=cv2.imread(f'./resized_segmentation_img.png',cv2.IMREAD_GRAYSCALE)
    ...
    cv2.imwrite(f"{VTON_IMG_UPLOAD_DIR}/seg_img.png",img_seg)
    img=cv2.resize(img_seg,(768,1024))
    cv2.imwrite(f'{VTON_IMG_UPLOAD_DIR}/HR-VITON-main/test/test/image/00001_00.jpg',img)

    # Generate grayscale semantic segmentation image
    terminnal_command =f"python get_seg_grayscale.py"
    os.system(terminnal_command)
    ...
    os.system(terminnal_command)
    terminnal_command =f"python get_densepose.py"
    os.system(terminnal_command)
